...
class LadderBot(discord.Client):
    ...
    async def setup_hook(self) -> None:
        # Temporary fix for older discord.py versions that don't have CommandTree.add_check
        try:
            self.tree.add_check(only_allowed)  # works on discord.py >= 2.4
        except AttributeError:
            print("⚠️ Skipping add_check: discord.py version too old")
